...
const StatusCard = ({ title, count, colorClass, icon: Icon }) => (
    <div
        className={`p-5 bg-white rounded-xl shadow-lg border-l-4 ${colorClass}`}
    >
...
export default function VendorManagement({ vendors = [] }) {
    // Mengambil Auth User & Flash Message dari props global Inertia
    ...
    // Effect untuk Notifikasi Flash dari Backend
    useEffect(() => {
        if (flash?.success) {
    ...
    // Derived State (Hitung Jumlah & Filter)
    ...
    const visibleVendors = useMemo(() => {
        return vendors.filter(
            (v) => (v.isApproved || "PENDING") === currentStatus
        );
    }, [vendors, currentStatus]);

    // Handlers
    const handleViewDetail = (vendor) =>
        setDetailModal({ isOpen: true, vendor });
                                    ...
                                    <tr>
                                        ...
                                        >
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p>
                                                    Tidak ada data vendor di
                                                    kategori {currentStatus}.
                                                </p>
                                                <p className="text-xs text-gray-400 mt-1">
                                                    Data baru akan muncul
                                                    otomatis di sini setelah
                                                    registrasi.
                                                </p>
                                            </div>
                                        </td>
