...
// Impor <Link> untuk transisi smooth
import { Link } from '@inertiajs/react';
import { FaEnvelope } from 'react-icons/fa'; // Hanya perlu ikon email
...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Ganti console.log dengan logika Inertia.post Anda
    console.log('Password reset requested for:', email);
    // Contoh: Inertia.post('/forgot-password', { email });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden lg:grid lg:grid-cols-2">
        ...
        <div className="p-8 lg:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-2">
              <button 
                type="submit"
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-full shadow-sm text-base font-medium text-white bg-gradient-to-r from-yellow-500 to-yellow-600 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-all"
              >
                Kirim Link Reset
              </button>
            </div>
        ...
        <div className="hidden lg:flex flex-col items-center justify-center p-12 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white text-center rounded-r-2xl">
          ...
          {/* Gunakan <Link> untuk transisi smooth */}
          <Link 
            href="/login" 
