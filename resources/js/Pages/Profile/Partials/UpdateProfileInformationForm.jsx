...
// --- KOMPONEN MOCK/KUSTOM (TETAP SAMA) ---
...
const Input = ({ className = "", type = "text", ...props }) => (
    <input
        type={type}
        className={`flex h-10 w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm ring-offset-white placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition duration-150 ease-in-out ${className}`}
        {...props}
    />
);
...
const Card = ({ children, className = "" }) => (
    <div
        className={`rounded-3xl border border-gray-100 bg-white text-gray-800 shadow-2xl shadow-gray-200/50 ${className}`}
    >
        {children}
    </div>
);
const CardHeader = ({ children, className = "" }) => (
    <div
        className={`flex flex-col space-y-1.5 p-6 rounded-t-3xl bg-gradient-to-r from-amber-50 to-yellow-50 border-b border-gray-100 ${className}`}
    >
        {children}
    </div>
);
const CardTitle = ({ children }) => (
    <h3 className="text-2xl md:text-3xl font-bold leading-none tracking-tight text-amber-700">
        {children}
    </h3>
);
...
export default function UpdateProfileInformationForm({
    ...
    const submit = (e) => {
        ...
        // Tentukan nama route berdasarkan role
        // Vendor -> 'vendor.profile.update'
        // Customer -> 'profile.update' (Default Laravel Breeze)
        const routeName = isVendor ? "vendor.profile.update" : "profile.update";

        // Mengirim permintaan PATCH ke rute yang sesuai
    ...
    return (
        <Card className={`w-full ${className}`}>
            <CardHeader>
                ...
                <CardDescription>
                    Perbarui informasi profil login (Nama & Email) akun Anda.
                </CardDescription>
            </CardHeader>
            <CardContent>
